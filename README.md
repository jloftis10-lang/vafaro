...
The lead and contact APIs require `DATABASE_URL`. Reviewer notifications activate when `RESEND_API_KEY` and `REVIEWER_NOTIFICATION_EMAIL` are configured; `VAFARO_NOTIFICATION_FROM` optionally sets a verified sender. Vercel Web Analytics is enabled through `@vercel/analytics`.
