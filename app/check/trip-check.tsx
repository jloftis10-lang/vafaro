...
export function TripCheck() {
  ...
  return <div className="check-shell shell">
    ...
    {step === 1 && <section className="question"><p className="section-kicker">FIRST, THE PEOPLE</p><h1>Who’s coming along?</h1><p>{profileRestored?"Your saved traveler profile is ready. Adjust anything that changed.":"Select everyone this trip needs to work for."}</p><div className="choice-grid">{companions.map(item => <button className={selected.includes(item) ? "choice selected" : "choice"} key={item} onClick={() => toggle(item, selected, setSelected)}>{item}{selected.includes(item) ? <Check size={16} weight="bold" /> : null}</button>)}</div></section>}
